# vim:ts=4:sw=4:ai:
# $Id: DEBUG.pm,v 1.1.1.1 2002/02/20 12:30:47 chiselwright Exp $

package UglyMUG::Log::Handler::DEBUG;
...
This is the B<DEBUG> specific entry handler.
It formats entries to look like:

  [TimeStamp] Message

This is then appended to the B<debug> log in the B<day> directory under B<admin>.
...
sub process ($) {
	...
	# DEBUG format: timestamp|message
	# ^UDEBUG^W1013696166^Wloop: currentline: @remote {@?location $exitin} = ;%h%c{@?name $shuttle_id} has arrived.%z^V$
	my (
		$timestamp,
	...
	$self->dprint(5, "DEBUG: $self->{'log_message'}\n");

	# and append to the debug log
	$self->append_admin_log('debug', 'day');
}
