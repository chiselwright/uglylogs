# vim:ts=4:sw=4:ai:
# $Id: WALL.pm,v 1.1.1.1 2002/02/20 12:30:47 chiselwright Exp $

package UglyMUG::Log::Handler::WALL;
...
This is the B<WALL> specific entry handler.
It formats entries to look like

  [TimeStamp] PlayerName(#id) shouted: A Message

This is then appended to the B<wall> log in the
...
sub process ($) {
	...
	# WALL format: timestamp|playerid|playername|message
	my (
		$timestamp,
		$playerid,
		$playername,
	...
	$self->dprint(5, "WALL: $self->{'log_message'}\n");

	# and append to the wall log
	$self->append_admin_log('wall', 'day');
}
