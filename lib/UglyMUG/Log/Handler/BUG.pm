# vim:ts=4:sw=4:ai:
# $Id: BUG.pm,v 1.1.1.1 2002/02/20 12:30:46 chiselwright Exp $

package UglyMUG::Log::Handler::BUG;
...
This is the B<BUG> specific entry handler.
It formats entries to look like

  [TimeStamp] BUG: A Message

This is then appended to the B<bug> log in the
...
sub process ($) {
	...
	$self->{'log_message'} = "[$self->{'timestr'}] BUG: $message";
	# maybe some debug information
	$self->dprint(5, "BUG: $self->{'log_message'}\n");

	# and append to the bug log
	$self->append_admin_log('bug', 'day');
}
