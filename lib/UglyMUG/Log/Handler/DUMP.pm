# vim:ts=4:sw=4:ai:
# $Id: DUMP.pm,v 1.1.1.1 2002/02/20 12:30:46 chiselwright Exp $

package UglyMUG::Log::Handler::DUMP;
...
This is the B<DUMP> specific entry handler.
It formats entries to look like

  [TimeStamp] DUMP: FileName (START|DONE)
...
sub process ($) {
	...
	# DUMP format: timestamp|file|<START|DONE>
	my (
		$timestamp,
		$file,
	...
	$self->{'log_message'} = "[$self->{'timestr'}] DUMP: $file $status";
	# maybe some debug information
	$self->dprint(5, "DUMP $self->{'log_message'}\n");
