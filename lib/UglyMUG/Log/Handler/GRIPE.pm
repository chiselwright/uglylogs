...
package UglyMUG::Log::Handler::GRIPE;
...
This is the B<GRIPE> specific entry handler.
It formats entries to look like

  [TimeStamp] GRIPE from PlayerName(#id) in LocationName(#id): A Message

This is then appended to the B<gripe> log in the B<day> directory under B<admin>.
The message is also appended to the B<allgripes> log in the B<keep> directory under B<admin>.
...
sub process ($) {
	...
	# GRIPE format: timestamp|playerid|playername|locationid|locationname|message
	my (
		$timestamp,
		$playerid,
		$playername,
	...
	$self->{'log_message'} = "[$self->{'timestr'}] GRIPE from $playername(#$playerid) in $locationname(#$locationid): $message";
	# maybe some debug information
	$self->dprint(5, "gripe: $self->{'log_message'}\n");

	# and append to the gripe log
	$self->append_admin_log('gripe', 'day');
	# we also append to a growing log of gripes which should only be
	# trimmed as the actual gripes are dealt with
	$self->append_admin_log('allgripes', 'keep');
}
