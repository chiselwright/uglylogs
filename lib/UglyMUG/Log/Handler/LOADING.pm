# vim:ts=4:sw=4:ai:
# $Id: LOADING.pm,v 1.1.1.1 2002/02/20 12:30:47 chiselwright Exp $

package UglyMUG::Log::Handler::LOADING;
...
This is the B<LOADING> specific entry handler.
It formats entries to look like

  [TimeStamp] LOADING: FileName (START|DONE)
...
sub process ($) {
	...
	# LOADING format: timestamp|file|<START|DONE>
	my (
		$timestamp,
		$file,
		$status) = split(/$self->{'separator'}{'field'}/, $self->{'entry'});
	...
	$self->{'log_message'} = "[$self->{'timestr'}] LOADING: $file $status";
	# maybe some debug information
	$self->dprint(5, "LOADING $self->{'log_message'}\n");
